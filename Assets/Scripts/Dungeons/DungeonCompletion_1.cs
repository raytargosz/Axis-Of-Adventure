...
public class DungeonCompletion : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player"))
        {
            ...
            // Make WorldSpace 1 and WorldSpace 2 visible if returning to the main scene
            if (GameManager.Instance.GetCurrentWorldSpace() == 1)
            {
                GameManager.Instance.worldSpaces[0].SetActive(true);
                GameManager.Instance.worldSpaces[1].SetActive(true);
            }
        }
    }
}